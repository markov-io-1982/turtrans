...
class Personnel {
    ...
    private function setCarrier() {
        ...
        if ($_SESSION['user']['position_id'] == 1) {
            ...
            $this->users_carrier_where = '1';
            $this->carrier_where = '1';
        }
        // this is carrier
        else if ($_SESSION['user']['position_id'] == 2) {
            $this->carrier_id = $_SESSION['user']['id'];
            $this->users_carrier_where = 'users.carrier_id = '.$this->carrier_id;
    ...
    private function loadpage() {
        $results = '';
        $sql = 'SELECT users.*, 
            (SELECT positions.name FROM positions WHERE users.position_id = positions.id) as position_name,
            (SELECT roles.name FROM roles WHERE users.role_id = roles.id) as role_name
            FROM users 
            LEFT JOIN positions ON users.position_id = positions.id
            LEFT JOIN roles ON users.role_id = roles.id
            WHERE users.position_id > 2 AND '.$this->users_carrier_where;
