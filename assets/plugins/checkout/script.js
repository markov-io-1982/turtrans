...
const ACTION_HANDLE_INPUT_CHANGE = "ACTION_HANDLE_INPUT_CHANGE";
...
function handleFormChange(item, name, value){
  store.dispatch({
    type: ACTION_HANDLE_INPUT_CHANGE,
    item,
    name,
    value });
};
...
function booking(state = data, action) {
  switch (action.type) {
    ...
    case ACTION_SEAT_CLICK:
      ...
      if (stateNew.reserved.length !== 0) {
        ...
        if (index >= 0) {
          ...
          stateNew.reserved.push({item: combRowSeat, priceItem: action.price, firstName: "", lastName: "", email: "", phone: ""});
        }
      } else {
        stateNew.reserved = [{item: combRowSeat, priceItem: action.price, firstName: "", lastName: "", email: "", phone: ""}];
      }
      return stateNew;
    ...
    case ACTION_ORDER:
      ...
      console.log("order", stateAfterOrder.reserved)
    ...
    case ACTION_HANDLE_INPUT_CHANGE:
      const it = action.item;
      const name = action.name;
      const value = action.value;
      const sta = cloneState(state);
      sta.reserved[it][name] = value;
      return sta;
...
class ReservedOneSeat extends React.Component {
  ...
  handleInputChange = e => {
    ...
    handleFormChange(this.props.item, name, value);
  };
      ...
      React.createElement("div", { className: "reserved-seat" },
        ...
        React.createElement("div", { className: "row" },
          ...
          React.createElement("div", { className: "col-sm-6" },
            ...
            React.createElement("input", { className: "form-control" , name: "firstName", onChange: this.handleInputChange}),
            
            !this.props.firstName && React.createElement("p", { className: "firstNameErr", style: {color: "red"}, name: "firstNameErr"}, this.props.err),
          ),

          React.createElement("div", { className: "col-sm-6" },
            React.createElement('label', {}, 'Прізвище'),
            React.createElement("input", { className: "form-control", name: "lastName", onChange: this.handleInputChange }),

            !this.props.lastName && React.createElement("p", { className: "lastNameErr", style: {color: "red"}, name: "lastNameErr"}, this.props.err)
          ),

          React.createElement("div", { className: "col-sm-6" },
            React.createElement('label', {}, 'Phone'),
            React.createElement("input", { className: "form-control", name: "phone", onChange: this.handleInputChange, type: "tel" }),

            !this.props.phone && React.createElement("p", { className: "phoneErr", style: {color: "red"}, name: "phoneErr"}, this.props.err)
          ),

          React.createElement("div", { className: "col-sm-6" },
            React.createElement('label', {}, 'Email'),
            React.createElement("input", { className: "form-control", name: "email",  onChange: this.handleInputChange, type: "email" }),

            !this.props.email && React.createElement("p", { className: "emailErr", style: {color: "red"}, name: "emailErr"}, this.props.err)
          ),
          ...
          React.createElement("div", { className: "col-sm-6 place-price" },
            React.createElement("label", { className: "wp-ticket-price-label" }, "Вартість квитка: ", priceTicket)),
    ...
    return (
      ...
      reserved.map(function (combRowSeat, i) {
        if(Object.entries(combRowSeat).length !== 0 || combRowSeat.constructor !== Object){
          ...
          const firstName = combRowSeat.firstName;
          const lastName = combRowSeat.lastName;
          const phone = combRowSeat.phone;
          const email = combRowSeat.email;
          
          total += price;                            

          return React.createElement(ReservedOneSeat, {rowNo: rowNo, seatNo: seatNo, err: err,
      ...
      React.createElement("span", { className: "reserved-total-label" }, "Загальна сума:"),      
...
class App extends React.Component {
  constructor(props) {
    super(props);
    this.state = { checkedOrder: "", chackedBook: "", err: "" };

  }

  handleBuy = (event) => {
    event.preventDefault();
    let check = this.checkOrder.checked;
    let reserved = store.getState().reserved.every((item) => {
      return item.firstName && item.lastName && item.phone && item.email;
    });

    if(check && reserved){
      order();
      this.setState({checkedOrder: "", err: ""});
    }else{
      this.setState({checkedOrder: "* Ви повинні погодитися з політикою сайту.", err: "* Поле не може бути пустим!"});
    };
  }

  handleBook = (event) => {
    event.preventDefault();
    let check = this.checkBook.checked;
    let reserved = store.getState().reserved.every((item) => {
      return item.firstName && item.lastName && item.phone && item.email;
    });
    if(check && reserved){
      order();
      this.setState({chackedBook: ""});
    }else{
      this.setState({chackedBook: "* Ви повинні погодитися з політикою сайту.", err: "* Поле не може бути пустим!"});
    };
  }
  
  render() {
    const reserved = store.getState().reserved;
    ...
    return (
      ...
      React.createElement(ReservedSeats, { reserved: store.getState().reserved, err: err}),
      React.createElement("div", null,
      ...
      React.createElement("div", { className: "tr-single-box" },
        React.createElement("div", {className: "tr-single-body"},
          ...
          // Оплатити
            React.createElement("header", {className: "payment-card-header cursor-pointer", "data-toggle": "collapse", "data-target": "#debit-credit", "aria-expanded": "true"}, 
              React.createElement("div", {className: "payment-card-title flexbox"},
                React.createElement("h4", {}, "Оплатити онлайн")
              ),
              React.createElement("div", {className: "pull-right"}, 
                React.createElement("img", {alt: "logo", src: "https://mlp-bus.com/img/visa.png"}),
                React.createElement("img", {alt: "logo", src: "https://mlp-bus.com/img/mastercard.png"})
              )
            ),
                ...
                React.createElement("div", {className: "booking-price-detail side-list no-border"}, 
                  React.createElement("ul", {},
                    React.createElement("li", {}, "Сума до оплати",
                      React.createElement("strong", {className: "theme-cl pull-right"}, total +" грн.")
                    )
                  )
                ),

                React.createElement("div", {className: "form", "data-validate": "parsley"}, 
                  React.createElement("div", {className: "wp-personal-data"},
                    React.createElement("input", {className: "form-control parsley-validated", ref: (checkOrder) => this.checkOrder = checkOrder, type: "checkbox", id: "check", name: "check", value: "option1",
                    "data-required": "true" }),
                    React.createElement("label", {htmlFor: "check"}, "Я приймаю ", 
                      React.createElement("a", {href: "personal-data.html", className: "btn-signup red-btn", target: "_blank"}, "умови обробки персональних даних")
                    ),
                    React.createElement("p", {style: {color: "red"}}, this.state.checkedOrder)
                  ),

                  React.createElement("div", {className: "wp-success"},
                    React.createElement("button", {type: "submit", className: "btn btn-m theme-btn", onClick: this.handleBuy}, "Оплатити")
                ))
              )
            )
          ),
          React.createElement("div", {className: "payment-card wp-types-of-payment wp-types-of-payment-booking"},


          // Забронювати
          React.createElement("header", {className: "payment-card-header cursor-pointer", "data-toggle": "collapse", "data-target": "#booking", "aria-expanded": "true"}, 
              React.createElement("div", {className: "payment-card-title flexbox"},
                React.createElement("h4", {}, "Забронювати")
              )
            ),

            React.createElement("div", {className: "collapse", id: "booking", "aria-expanded": false}, 
              React.createElement("div", {className: "payment-card-body"},
                React.createElement("div", {className: "booking-price-detail side-list no-border"}, 
                  React.createElement("p", {}, "При бнонюванні квитка, Вам буде надіслано повідомлення на email, перевірте пошту та перейдіть по ссилці щоб оплатити заброньований квиток. Квиток дійсний 12 годин.")
                ),

                React.createElement("div", {className: "form", "data-validate": "parsley"}, 
                  React.createElement("div", {className: "wp-personal-data"},
                    React.createElement("input", {type: "checkbox", id: "checkBooking", ref: (checkBook) => this.checkBook = checkBook, name: "checkBooking", value: "option1",
                    "data-required": "true", className: "parsley-validated"}),
                    React.createElement("label", {htmlFor: "checkBooking"}, "Я приймаю ", 
                      React.createElement("a", {href: "personal-data.html", className: "btn-signup red-btn", target: "_blank"}, "умови обробки персональних даних")
                    ),
                    React.createElement("p", {style: {color: "red"}}, this.state.chackedBook)
                  ),

                  React.createElement("div", {className: "wp-success"},
                    React.createElement("button", {type: "submit", className: "btn btn-m theme-btn", onClick: this.handleBook}, "Забронювати")
                ))
              )
            )
          )
